...
public class UserAccessResource {
    ...
    public ResponseEntity<UserAccessDTO> createUserAccess(@RequestBody UserAccessDTO userAccessDTO) throws URISyntaxException {
        ...
        if (userAccessDTO.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new userAccess cannot already have an ID")).body(null);
        }
    ...
    public ResponseEntity<UserAccessDTO> updateUserAccess(@RequestBody UserAccessDTO userAccessDTO) throws URISyntaxException {
        ...
        if (userAccessDTO.getId() == null) {
            return createUserAccess(userAccessDTO);
        }
        UserAccessDTO result = userAccessService.save(userAccessDTO);
    ...
    @Timed
    public List<UserAccessDTO> getAllUserAccesses() {
        log.debug("REST request to get all UserAccesses");
    ...
    public ResponseEntity<UserAccessDTO> getUserAccess(@PathVariable Long id) {
        ...
        UserAccessDTO userAccessDTO = userAccessService.findOne(id);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(userAccessDTO));
    }
    ...
    /**
     * SEARCH  /_search/user-accesses?query=:query : search for the userAccess corresponding
     * to the query.
     *
     * @param query the query of the userAccess search
     * @return the result of the search
     */
    @GetMapping("/_search/user-accesses")
    @Timed
    public List<UserAccessDTO> searchUserAccesses(@RequestParam String query) {
        log.debug("REST request to search UserAccesses for query {}", query);
        return userAccessService.search(query);
    }
