...
@Component({
    selector: 'jhi-sinister-prestation-view',
    templateUrl: './sinister-prestation-view.component.html'
})
export class SinisterPrestationViewComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.route.params.subscribe((params) => {
            if (this.prestationId) {
                ...
                console.log("yes work " + this.prestationId);

                this.sinisterService.findByPrestation(this.prestationId).subscribe((sinister: Sinister) => { // Edition
                    ...
                    for (let i = 0; i < this.sinister.prestations.length; i++) {
                        if (this.prestationId === this.sinister.prestations[i].id) {
                            this.sinisterPrestation = this.sinister.prestations[i];
                            console.log("testLengthSinisterPrestation " + this.sinisterPrestation.observations.length);
                            ...
                            if (!this.sinisterPrestation.gageo) {
                                ...
                                this.governorateService.queryGaGeo().subscribe((res: ResponseWrapper) => { this.governorates = res.json; }, (res: ResponseWrapper) => this.onError(res.json));
                                this.delegationService.queryGaGeo().subscribe((res: ResponseWrapper) => { this.cities = res.json; }, (res: ResponseWrapper) => this.onError(res.json));
                            }
