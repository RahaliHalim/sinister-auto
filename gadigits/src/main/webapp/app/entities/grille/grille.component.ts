...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/grille'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Grille) {
        return item.id;
    }
    registerChangeInGrilles() {
        this.eventSubscriber = this.eventManager.subscribe('grilleListModification', (response) => this.loadAll());
    }
