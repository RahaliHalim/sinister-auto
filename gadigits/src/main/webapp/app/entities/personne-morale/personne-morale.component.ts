 ...
 camions: Camion[];
    ...
    loadAll() {
        if (this.currentSearch) {
            this.personneMoraleService.search({
    ...
    transition() {
        this.router.navigate(['/personne-morale'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    private onSuccessContact(data, headers) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = headers.get('X-Total-Count');
        this.queryCount = this.totalItems;
        this.contacts = data;
    }
    private onSuccessCamion(data, headers) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = headers.get('X-Total-Count');
        this.queryCount = this.totalItems;
        this.camions = data;
    ...
    trackId(index: number, item: PersonneMorale) {
        return item.id;
    }
    registerChangeInPersonneMorales() {
        this.eventSubscriber = this.eventManager.subscribe('personneMoraleListModification', (response) => this.loadAll());
    }
