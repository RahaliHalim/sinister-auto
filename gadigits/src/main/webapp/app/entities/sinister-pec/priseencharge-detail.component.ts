...
import { NgbModalRef, NgbDateStruct, NgbDateParserFormatter } from '@ng-bootstrap/ng-bootstrap';
...
import { HistoryService, HistoryPec } from '../history';
import { saveAs } from 'file-saver/FileSaver';
import { DevisService } from '../devis';
import { DomSanitizer } from '@angular/platform-browser';
@Component({
    selector: 'jhi-sinister-reprise-dialog',
    templateUrl: './priseencharge-detail.component.html'
})
export class PriseEnChargeDetailComponent implements OnInit {
    ...
    quotationModifPrix: Quotation = new Quotation();
    ...
    accordComplPreview2 = false;
    accordComplPreview3 = false;
    accordComplPreview4 = false;
    ...
    detailsPiecesMOModifPrix: DetailsPieces[] = [];
    ...
    lastDetailsPiecesModifPrix: DetailsPieces[] = [];
    ...
    lastDetailsModifPrix: DetailsPieces[] = [];
    ...
    detailsPiecesModifPrix: DetailsPieces[] = [];
    detailsPiecesFournitureModifPrix: DetailsPieces[] = [];
    ...
    moTotalTtcRevue = 0;
    ingTotalTtcRevue = 0;
    detailsPiecesIngredientModifPrix: DetailsPieces[] = [];
    ...
    listDevis: HistoryPec[] = [];
    listDevisTest: HistoryPec[] = [];
    testt: any;
    ...
    listAccord: HistoryPec[] = [];
    piecesJointess: PieceJointe[] = []
    ...
    ngOnInit() {
        ...
        this.refCompagnieService.findAllCompanies().subscribe((res: ResponseWrapper) => { this.compagnies = res.json; }, (res: ResponseWrapper) => this.onError(res.json));
        ...
        this.refModeGestionService.query().subscribe(
            ...
            (res: ResponseWrapper) => {
                ...
                this.listModeByCnvByUser = res.json;
            }
        )
                ...
                this.prestationPECService.findPrestationPec(this.pecId).subscribe((pecRes: SinisterPec) => {
                    ...
                    if (this.sinisterPec.id !== null && this.sinisterPec.id !== undefined) {
                        ...
                        this.historyService.findListDevisByHistoryPec(this.sinisterPec.id).subscribe((res) => {
                            ...
                            this.listDevis.sort(function (a, b) {
                                return a.id - b.id;
                            });
                        ...
                        this.historyService.findListAccordByHistoryPec(this.sinisterPec.id).subscribe((res) => {
                            ...
                            this.listAccord.sort(function (a, b) {
                                return a.id - b.id;
                            });
                        ...
                        this.prestationPECService.findAccordValidBySinisterPec(this.pecId).subscribe((resApec) => {
                            this.apecsValid = resApec.json;
    ...
    calculateGlobalPiecesTtcRevue() {
        this.quotationRevue.ttcAmount = this.quotationRevue.stampDuty ? this.quotationRevue.stampDuty : 0;
        this.piecesTotalTtcRevue = 0;
        if (this.detailsPiecesRevue && this.detailsPiecesRevue.length > 0) {
            for (let i = 0; i < this.detailsPiecesRevue.length; i++) {
                //if (this.detailsPieces[i].modified) {
                this.piecesTotalTtcRevue += this.detailsPiecesRevue[i].totalTtc;
                //}
            }
        }
        this.piecesTotalTtcRevue = +(this.piecesTotalTtcRevue).toFixed(3);
        this.quotationRevue.ttcAmount += this.moTotalTtcRevue + this.ingTotalTtcRevue + this.fournTotalTtcRevue + this.piecesTotalTtcRevue;
        this.quotationRevue.ttcAmount = +(this.quotationRevue.ttcAmount).toFixed(3);
    }


    calculateGlobalPiecesTtcModifPrix() {
        this.quotationModifPrix.ttcAmount = this.quotationModifPrix.stampDuty ? this.quotationModifPrix.stampDuty : 0;
        this.piecesTotalTtcModifPrix = 0;
        if (this.detailsPiecesModifPrix && this.detailsPiecesModifPrix.length > 0) {
            for (let i = 0; i < this.detailsPiecesModifPrix.length; i++) {
                //if (this.detailsPieces[i].modified) {
                this.piecesTotalTtcModifPrix += this.detailsPiecesModifPrix[i].totalTtc;
                //}
            }
        }
        this.piecesTotalTtcModifPrix = +(this.piecesTotalTtcModifPrix).toFixed(3);
        this.quotationModifPrix.ttcAmount += this.moTotalTtcModifPrix + this.ingTotalTtcModifPrix + this.fournTotalTtcModifPrix + this.piecesTotalTtcModifPrix;
        this.quotationModifPrix.ttcAmount = +(this.quotationModifPrix.ttcAmount).toFixed(3);
    }
    ...
    getPieceNewIOOB(entityName: string, sinisterPecId: number, label: string) {
        this.prestationPECService.getPieceBySinPecAndLabelIOOB(entityName, sinisterPecId, label).subscribe((blob: Blob) => {
            const indexOfFirst = (blob.type).indexOf('/');
            this.extensionImage = ((blob.type).substring((indexOfFirst + 1), ((blob.type).length)));
            this.extensionImage = this.extensionImage.toLowerCase();
            let fileName = label + "." + this.extensionImage;
