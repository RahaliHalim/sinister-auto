...
@Component({
    selector: 'jhi-apec',
    templateUrl: './apec-a-approuver.component.html'
})
export class ApecApprouveComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        ...
        this.apecService.queryByStatus(EtatAccord.ACC_A_APPROUV).subscribe(
    ...
    ngOnInit() {
        ...
        this.principal.identity().then((account) => {
            ...
            this.userExtraService.findFunctionnalityEntityByUser(55, this.currentAccount.id).subscribe(res => {
                ...
                console.log("permission to access****accord"+this.permissionToAccess);
            });
        });
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < this.apecs.length; i++) {
            if(this.apecs[i].sinisterPec.stepId == 100){
