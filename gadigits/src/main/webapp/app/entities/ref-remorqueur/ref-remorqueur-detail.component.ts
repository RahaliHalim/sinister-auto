...
export class RefRemorqueurDetailComponent implements OnInit {
    bloque = false;
    list = '';
    list1: number[] = [];
    disabledMotif = false;
            ...
            if (params['idRmq']) {
                this.camionService.findByRefRemorqueur(params['idRmq']).subscribe((res: ResponseWrapper) => {
                    ...
                    this.listCamions.forEach(camion => {
                        ...
                        console.log("iciiiiiii loooooooooooooooooooooookkkkkk" + camion.serviceTypesStr)
    ...
    private onPerSaveSuccess(result: PersonneMorale) {
        ...
        if (this.refRemorqueur.id !== undefined) {
            if (this.listContacts.length > 0) {
                ...
                this.refRemorqueur.telephone = this.visAVis.premTelephone;
            }
