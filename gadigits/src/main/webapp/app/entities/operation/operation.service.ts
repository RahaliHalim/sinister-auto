...
export class OperationService {
    ...
    findForAssistance(req?: any): Observable<ResponseWrapper> {
        const options = createRequestOption(req);
        return this.http.get(`${this.resourceUrl}/assistance`, options)
            .map((res: Response) => this.convertResponse(res));
    }

    findForPec(req?: any): Observable<ResponseWrapper> {
        const options = createRequestOption(req);
        return this.http.get(`${this.resourceUrl}/pec`, options)
            .map((res: Response) => this.convertResponse(res));
    }
