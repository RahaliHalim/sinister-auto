...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.personnePhysiqueService.search({
        ...
        this.personnePhysiqueService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
            (res: ResponseWrapper) => this.onSuccess(res.json, res.headers),
    ...
    transition() {
        this.router.navigate(['/personne-physique'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: PersonnePhysique) {
        return item.id;
    }
    registerChangeInPersonnePhysiques() {
        this.eventSubscriber = this.eventManager.subscribe('personnePhysiqueListModification', (response) => this.loadAll());
    }
