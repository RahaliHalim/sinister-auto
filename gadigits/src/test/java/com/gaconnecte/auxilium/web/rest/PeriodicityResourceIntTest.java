...
/**
 * Test class for the PeriodicityResource REST controller.
 *
 * @see PeriodicityResource
 */
@RunWith(SpringRunner.class)
@SpringBootTest(classes = AuxiliumApp.class)
public class PeriodicityResourceIntTest {
    ...
    @Autowired
    private PeriodicitySearchRepository periodicitySearchRepository;
    ...
    public void setup() {
        ...
        PeriodicityResource periodicityResource = new PeriodicityResource(periodicityService);
    ...
    public void initTest() {
        periodicitySearchRepository.deleteAll();
    ...
    public void createPeriodicity() throws Exception {
        ...
        assertThat(testPeriodicity.getCode()).isEqualTo(DEFAULT_CODE);
        assertThat(testPeriodicity.getLabel()).isEqualTo(DEFAULT_LABEL);
        assertThat(testPeriodicity.isActive()).isEqualTo(DEFAULT_ACTIVE);

        // Validate the Periodicity in Elasticsearch
        Periodicity periodicityEs = periodicitySearchRepository.findOne(testPeriodicity.getId());
        assertThat(periodicityEs).isEqualToComparingFieldByField(testPeriodicity);
    }
    ...
    public void updatePeriodicity() throws Exception {
        ...
        Periodicity updatedPeriodicity = periodicityRepository.findOne(periodicity.getId());
        updatedPeriodicity
            .code(UPDATED_CODE)
            .label(UPDATED_LABEL)
        ...
        Periodicity testPeriodicity = periodicityList.get(periodicityList.size() - 1);
        assertThat(testPeriodicity.getCode()).isEqualTo(UPDATED_CODE);
    ...
    public void updateNonExistingPeriodicity() throws Exception {
        ...
        // Validate the Periodicity in the database
        List<Periodicity> periodicityList = periodicityRepository.findAll();
        assertThat(periodicityList).hasSize(databaseSizeBeforeUpdate + 1);
    }
    ...
    public void deletePeriodicity() throws Exception {
        ...
        boolean periodicityExistsInEs = periodicitySearchRepository.exists(periodicity.getId());
        assertThat(periodicityExistsInEs).isFalse();
    ...
    @Test
    @Transactional
    public void searchPeriodicity() throws Exception {
        // Initialize the database
        periodicityRepository.saveAndFlush(periodicity);
        periodicitySearchRepository.save(periodicity);

        // Search the periodicity
        restPeriodicityMockMvc.perform(get("/api/_search/periodicities?query=id:" + periodicity.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(periodicity.getId().intValue())))
            .andExpect(jsonPath("$.[*].code").value(hasItem(DEFAULT_CODE.toString())))
