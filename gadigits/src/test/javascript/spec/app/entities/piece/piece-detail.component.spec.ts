/* tslint:disable max-line-length */
import { ComponentFixture, TestBed, async, inject } from '@angular/core/testing';
import { OnInit } from '@angular/core';
import { DatePipe } from '@angular/common';
import { ActivatedRoute } from '@angular/router';
import { Observable } from 'rxjs/Rx';
import { JhiDateUtils, JhiDataUtils, JhiEventManager } from 'ng-jhipster';
import { AuxiliumTestModule } from '../../../test.module';
import { MockActivatedRoute } from '../../../helpers/mock-route.service';
import { PieceDetailComponent } from '../../../../../../main/webapp/app/entities/piece/piece-detail.component';
import { PieceService } from '../../../../../../main/webapp/app/entities/piece/piece.service';
import { Piece } from '../../../../../../main/webapp/app/entities/piece/piece.model';

describe('Component Tests', () => {

    describe('Piece Management Detail Component', () => {
        let comp: PieceDetailComponent;
        let fixture: ComponentFixture<PieceDetailComponent>;
        let service: PieceService;

        beforeEach(async(() => {
            TestBed.configureTestingModule({
                imports: [AuxiliumTestModule],
                declarations: [PieceDetailComponent],
                providers: [
                    JhiDateUtils,
                    JhiDataUtils,
                    DatePipe,
                    {
                        provide: ActivatedRoute,
                        useValue: new MockActivatedRoute({id: 123})
                    },
                    PieceService,
                    JhiEventManager
                ]
            }).overrideTemplate(PieceDetailComponent, '')
            .compileComponents();
        }));

        beforeEach(() => {
            fixture = TestBed.createComponent(PieceDetailComponent);
            comp = fixture.componentInstance;
            service = fixture.debugElement.injector.get(PieceService);
        });

        describe('OnInit', () => {
            it('Should call load all on init', () => {
            // GIVEN

            spyOn(service, 'find').and.returnValue(Observable.of(new Piece(10)));

            // WHEN
            comp.ngOnInit();

            // THEN
            expect(service.find).toHaveBeenCalledWith(123);
            expect(comp.piece).toEqual(jasmine.objectContaining({id: 10}));
            });
        });
    });

});
