/* tslint:disable max-line-length */
import { ComponentFixture, TestBed, async, inject } from '@angular/core/testing';
import { OnInit } from '@angular/core';
import { DatePipe } from '@angular/common';
import { ActivatedRoute } from '@angular/router';
import { Observable } from 'rxjs/Rx';
import { JhiDateUtils, JhiDataUtils, JhiEventManager } from 'ng-jhipster';
import { AuxiliumTestModule } from '../../../test.module';
import { MockActivatedRoute } from '../../../helpers/mock-route.service';
import { PersonneMoraleDetailComponent } from '../../../../../../main/webapp/app/entities/personne-morale/personne-morale-detail.component';
import { PersonneMoraleService } from '../../../../../../main/webapp/app/entities/personne-morale/personne-morale.service';
import { PersonneMorale } from '../../../../../../main/webapp/app/entities/personne-morale/personne-morale.model';

describe('Component Tests', () => {

    describe('PersonneMorale Management Detail Component', () => {
        let comp: PersonneMoraleDetailComponent;
        let fixture: ComponentFixture<PersonneMoraleDetailComponent>;
        let service: PersonneMoraleService;

        beforeEach(async(() => {
            TestBed.configureTestingModule({
                imports: [AuxiliumTestModule],
                declarations: [PersonneMoraleDetailComponent],
                providers: [
                    JhiDateUtils,
                    JhiDataUtils,
                    DatePipe,
                    {
                        provide: ActivatedRoute,
                        useValue: new MockActivatedRoute({id: 123})
                    },
                    PersonneMoraleService,
                    JhiEventManager
                ]
            }).overrideTemplate(PersonneMoraleDetailComponent, '')
            .compileComponents();
        }));

        beforeEach(() => {
            fixture = TestBed.createComponent(PersonneMoraleDetailComponent);
            comp = fixture.componentInstance;
            service = fixture.debugElement.injector.get(PersonneMoraleService);
        });

        describe('OnInit', () => {
            it('Should call load all on init', () => {
            // GIVEN

            spyOn(service, 'find').and.returnValue(Observable.of(new PersonneMorale(10)));

            // WHEN
            comp.ngOnInit();

            // THEN
            expect(service.find).toHaveBeenCalledWith(123);
            expect(comp.personneMorale).toEqual(jasmine.objectContaining({id: 10}));
            });
        });
    });

});
